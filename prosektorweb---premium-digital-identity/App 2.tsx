...
import { analyzeIndustry } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      ...
      setAnalysis(result || "Analiz yapılamadı, lütfen tekrar deneyin.");
    } catch (error) {
      console.error(error);
      setAnalysis("AI servisine şu an ulaşılamıyor. Lütfen daha sonra deneyin.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-orange-500/30 overflow-x-hidden">
      ...
      >
        <div className="space-y-6">
          <p className="text-gray-400">Hangi sektörde faaliyet gösterdiğinizi yazın, Gemini AI sizin için dijital strateji önerilerini hazırlasın.</p>
