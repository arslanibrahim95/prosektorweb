...
function generateServiceDefinitionSection(
  ...
  const provinceSectors = getProvinceSectorNames(province.id, 3);
  const sectorContext = provinceSectors.length
    ? `${provinceSectors.join(", ")} sektorlerinde uzman kadro`
    : "Bolgesel sektorlerde uzman kadro";
...
function generateLocalBusinessSchema(
  ...
  return {
    ...
    knowsAbout: getProvinceSectorNames(province.id, 3).map(
      (s) => `${s} ISG`
    ),
    priceRange: "$$",
  };
...
export function generateLocalPage(
  ...
  const metaDescription = generateMetaDescription(service, province.name, district?.name, sectorNames);
  const keywords = generateLocationKeywords(service, province.name, district?.name, sectorNames[0]);
  const h1 = district
