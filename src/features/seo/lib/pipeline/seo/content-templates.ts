...
export interface TemplateVariables {
  ...
  // Industry/Sector
  bolge_sektorler?: string; // "Otomotiv Sanayi, Kimya Sanayi"
  bolge_sektorler_kucuk?: string; // "otomotiv, kimya"
  bolge_riskler?: string; // "makine sikisma, kimyasal maruziyet"
...
export function prepareVariables(
  ...
  const sectorNames = getProvinceSectorNames(province.id, 3);
  const sectorRisks = getProvinceSectorRisks(province.id);
  ...
  return {
    ...
    bolge_sektorler: sectorNames.length ? sectorNames.join(", ") : undefined,
    bolge_sektorler_kucuk: sectorNames.length
      ? sectorNames.map((s) => s.toLowerCase()).join(", ")
      : undefined,
    bolge_riskler: sectorRisks.length ? sectorRisks.join(", ") : undefined,
...
{{bolge_sektorler}} sektorlerinde uzmanlasmis kadromuzla hizmetinizdeyiz.

[Ucretsiz Teklif Alin](#iletisim) | [Hemen Arayin](tel:{{telefon}})
      `.trim(),
      variables: ["bolge", "telefon", "bolge_sektorler"],
    },
    {
      id: "hizmet_tanimi",
      title: "Hizmet Tanimi",
...
{{sehir}} bolgesinde ozellikle {{bolge_sektorler_kucuk}} sektorlerinde yogun olarak hizmet veriyoruz.
{{bolge_riskler}} gibi sektore ozel risklere karsi saglik gozetimi yapiyoruz.
...
{{bolge_sektorler}} sektorlerinde deneyimli uzman kadromuz hizmetinizde.
...
{{bolge_sektorler}} sektorlerine ozel risk analizi deneyimimizle hizmetinizdeyiz.

[Teklif Alin](#iletisim)
      `.trim(),
      variables: ["bolge", "bolge_sektorler"],
    },
    {
      id: "risk_tanimi",
