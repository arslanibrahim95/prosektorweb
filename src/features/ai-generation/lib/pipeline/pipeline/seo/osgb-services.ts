...
export const OSGB_SERVICES: OsgbService[] = [
  ...
  // =========================================
  {
    ...
    targetSectors: [
      "otomotiv", "kimya", "metal", "insaat", "gida", "tekstil", "maden",
    ],
    legalReferences: [
  ...
  },
  {
    ...
    targetSectors: [
      "otomotiv", "kimya", "insaat", "metal", "maden", "tekstil", "gida",
    ],
    legalReferences: [
      "6331 sayili Is Sagligi ve Guvenligi Kanunu",
...
export function generateLocationKeywords(
  ...
  for (const pattern of service.locationKeywordPatterns) {
    ...
    if (keyword.includes("{sektor}")) {
      if (!sectorName) continue;
      keyword = keyword.replace("{sektor}", sectorName.toLowerCase());
    }
...
export function generateMetaDescription(
  ...
  const sectorSuffix = sectorNames?.length
    ? ` ${sectorNames.slice(0, 2).join(", ")} sektorlerine ozel.`
    : "";

  return `${location} bolgesinde profesyonel ${service.name.toLowerCase()} hizmeti.${sectorSuffix} ${service.shortDescription}. Hemen teklif alin!`;
}
