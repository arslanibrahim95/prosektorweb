...
import { z } from 'zod'
// Server-only imports - these modules use next/headers
// Note: Using dynamic imports to avoid client-side import issues
import type { OriginCheckResult } from './csrf'
...
export function safeApi<TResult, TParams = any>(
    ...
    return async (req: NextRequest, { params }: { params: Promise<TParams> }): Promise<NextResponse> => {
        ...
        // Lazy import getClientIp to avoid issues with client-side imports
        ...
        try {
            ...
            if (options.checkCsrf && !['GET', 'HEAD', 'OPTIONS'].includes(req.method)) {
                // Lazy import checkOrigin to avoid issues with client-side imports
                const { checkOrigin } = await import('./csrf')
                const originResult = await checkOrigin()
