...
export const GET = safeApi(async (request, { requestId }) => {
    ...
    // Redis distributed lock (prevent overlapping runs)
    const lockKey = 'cron:lock:renewals'
    const isLocked = await redis.set(lockKey, 1, { ex: 1800, nx: true }) // 30 min lock

    if (!isLocked) {
        return { success: false, error: 'Another cron process is currently running' }
    }
    ...
    try {
        ...
        return {
            data: results,
            stats,
        } as any // Bypass strict inference for now to unblock build
    } finally {
        // Release lock
